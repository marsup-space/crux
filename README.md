...
- `CRUX_VERSION=v0.8.1` 锁定版本（默认 `latest`）
...
当前版本：**0.8.1**。项目已经进入日常可用状态，仍保持快速迭代。
...
- `CRUX_VERSION=v0.8.1` — pin a version (default: `latest`)
...
Current version: **0.8.1**. Crux is now considered usable for daily work, while still moving quickly.
