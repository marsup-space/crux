...
- `CRUX_VERSION=v0.7.2` 锁定版本（默认 `latest`）
...
当前版本：**0.7.2**。项目已经进入日常可用状态，仍保持快速迭代。
...
- `CRUX_VERSION=v0.7.2` — pin a version (default: `latest`)
...
Current version: **0.7.2**. Crux is now considered usable for daily work, while still moving quickly.
