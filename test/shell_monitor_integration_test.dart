...
void main() {
  ...
  group('shell progress monitor — STUCK kills the process', () {
    test('a STUCK verdict kills a long-running command early', () async {
      ...
      final result = await tool.execute(
        ...
        callArgs('sleep 60'),
        ctx(monitor: (messages, {required abort}) async {
          evaluatorCalls++;
  ...
  group('shell progress monitor — no-aux regime unchanged', () {
    test('classic timeout still fires without a monitor evaluator',
        () async {
      final tool = BashTool();
      final sw = Stopwatch()..start();
    ...
    test('a PROGRESS verdict lets the command run to completion',
        () async {
      final tool = BashTool();
      var evaluatorCalls = 0;
