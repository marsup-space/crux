...
double computeTokPerSec({
  ...
  genMs +=
      DateTime.now().difference(roundFirstTokenTime).inMicroseconds /
      1000.0;
...
void main() {
  group('tok/s math (mirrors StreamingController.updateLiveMetrics)', () {
    ...
    test(
        'multi-round WITHOUT cumulativeGenMs folding: reproduces thousands '
      ...
      expect(
        rate,
        greaterThan(2000),
        reason: 'without folding elapsed time into cumulativeGenMs, '
    ...
    test(
        'CumulativeGenMs folds across many rounds: 10 rounds × 1s each '
