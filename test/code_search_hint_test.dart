// Tests for the one-shot `code_search` preference hint.
//
...
//      prefer code_search]` marker. Stable string contract so
...
//      shell-tool fallback guard's code_search verdict (which
//      fires on `rg | head` bash pipelines) — this hint fires
//      when the LLM uses grep/glob DIRECTLY as a tool.
//
//   3. **State semantics** — the `hasShownCodeSearchHint` flag
...
void main() {
  group('renderCodeSearchHintEmbedded', () {
    test('returns a non-empty string', () {
      final out = renderCodeSearchHintEmbedded();
      expect(out, isNotEmpty);
    });

    test('wraps the body in the `[Crux system note — prefer code_search]` marker',
        () {
      final out = renderCodeSearchHintEmbedded();
      expect(out, contains(codeSearchHintMarker));
    });

    test('mentions code_search by name (semantic search)', () {
      final out = renderCodeSearchHintEmbedded();
    ...
    test('is idempotent (same input → same output, every call)', () {
      ...
      expect(renderCodeSearchHintEmbedded(), renderCodeSearchHintEmbedded());
    });
  });
  ...
  group('injection rule (executed via the chat_service mutation shape)', () {
    ...
    test('appends hint to the first successful grep result', () {
      ...
      expect(out.results['b']!.output, isNot(contains(renderCodeSearchHintEmbedded())));
    ...
    test('does NOT append hint when flag is already true', () {
      ...
      expect(out.results['a']!.output, isNot(contains(renderCodeSearchHintEmbedded())));
    ...
    test('only the FIRST grep/glob gets the hint (subsequent ones don\'t)',
      ...
      expect(out.results['b']!.output, isNot(contains(renderCodeSearchHintEmbedded())));
      expect(out.results['c']!.output, isNot(contains(renderCodeSearchHintEmbedded())));
      expect(out.flag, isTrue);
    });
    ...
    test('does NOT append hint for non-trigger tools (read / write / bash)',
      ...
      for (final r in out.results.values) {
        expect(r.output, isNot(contains(renderCodeSearchHintEmbedded())));
      }
    ...
    test('mixed round: hint fires on the first grep, not on later read/code_search',
        () {
      // LLM called grep, read, code_search all in one round. Hint
      // fires on grep; the others (including code_search, which is
  ...
  group('SessionRuntimeState.hasShownCodeSearchHint', () {
    test('defaults to false on a freshly-constructed runtime', () {
      final rt = SessionRuntimeState(sessionId: 1);
      expect(rt.hasShownCodeSearchHint, isFalse);
    });

    test('can be flipped to true after the first grep/glob success', () {
      final rt = SessionRuntimeState(sessionId: 1);
      rt.hasShownCodeSearchHint = true;
      expect(rt.hasShownCodeSearchHint, isTrue);
    });
  ...
  group('MessageStore — appended hint persists in the tool result', () {
    ...
    test('the hint text round-trips through addToolRound when appended', () async {
      ...
      final augmentedOutput = rawOutput + renderCodeSearchHintEmbedded();
      ...
      expect(toolResult.content, contains(codeSearchHintMarker));
      ...
      expect(
        ...
        lessThan(toolResult.content.indexOf(codeSearchHintMarker)),
      );
    });
  });
}