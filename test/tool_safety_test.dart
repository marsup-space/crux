// Regression tests for LLM-tooling foot-guns observed in earlier
// sessions, plus the offload-pipeline behavior for `edit`:
//
...
//      callId so the LLM has a way to recover the bytes. This
//      still applies to `edit` (which offloads `oldString` /
//      `newString`); `write` no longer offloads at all (see
//      `write_tool.dart` for rationale).
//
...
void main() {
  group('compressCallForPersistence stand-in format', () {
    ...
    test(
      'write.content is never off-loaded (opted out of the pipeline)',
      () async {
        // Regression guard for the "no small-write escape hatch"
        // decision. WriteTool no longer extends LargePayloadTool, so
        // even very large write.content payloads pass through
        // compressCallForPersistence unchanged. The LLM needs the
        // full content to reference what it just wrote on
        // subsequent turns (especially for new files, where it has
        // no other anchor for what it produced).
        ...
        // Content is returned verbatim — no stand-in substitution.
        expect(compressed.input['content'], largeContent);
        // No offloaded_content row exists for the call.
        expect(
  ...
  group(
    'WriteTool size guard (regression: session 1208 silent truncation)',
    () {
      ...
      test(
        'guard does NOT fire when the new content is similar in size',
        () async {
      ...
      test(
        'force:true bypasses the guard and proceeds with the write',
        () async {
  ...
  group('EditTool result message includes the offload note', () {
    ...
    test('small edit (no offload) does not append the offload note', () async {
      ...
      expect(
        result.output,
        isNot(contains('offloaded_content')),
        reason: 'no offload happened, no note should appear',
      );
    });

    test(
      'large newString triggers an offload note for newString only',
      () async {
        ...
        expect(
          result.output,
          contains('call_edit_x_newString'),
    ...
    test(
      'both oldString and newString over the threshold are both listed',
      () async {
  ...
  group('WriteTool result message (no offload at any size)', () {
    late Directory tempDir;
    ...
    test('small write does not append the offload note', () async {
      final filePath = '${tempDir.path}/small.dart';
      final result = await WriteTool().execute(
        {'filePath': filePath, 'content': 'hello', 'intent': 'create new file'},
        ToolContext(
          sessionId: 1,
          messageId: 1,
          abort: AbortSignal(),
    ...
    test('large write does not append the offload note either '
        '(write opted out of the offload pipeline entirely)', () async {
      // WriteTool no longer extends LargePayloadTool, so even
      // very large write payloads keep their full content in
      // the conversation context. The result message must not
      // mention `offloaded_content` or any composite key — the
      // LLM doesn't need to recall anything.
      final filePath = '${tempDir.path}/big.dart';
      final result = await WriteTool().execute(
        {
          'filePath': filePath,
      ...
      expect(
        result.output,
        isNot(contains('offloaded_content')),
        reason: 'write never offloads; no recall note should appear',
      );
      expect(
        result.output,
        isNot(contains('call_w_big_content')),
        reason: 'no composite key should appear in the result',
      );
    });
  });
  ...
  group('Offload stand-in write guard', () {
    ...
    test(
      'edit refuses to use an offload stand-in as replacement text',
      () async {
    ...
    test(
      'edit can still target an existing stand-in in oldString for cleanup',
      () async {
  ...
  group('WriteTool read-before-write guard (regression: session 1141)', () {
    ...
    test(
      'read then write on a pre-existing file does not trigger guard',
      () async {
    ...
    test(
      'EditTool guard on a pre-existing unread file does not modify file',
      () async {
    ...
    test(
      'guard does NOT trigger for a non-existent file (new file creation)',
      () async {
    ...
    test(
      'session 1141 exact sequence: guard → read → write should succeed',
      () async {
    ...
    test('session 1141 exact sequence: guard auto-read → write should succeed '
      ...
      expect(
        ...
        contains('File written:'),
        reason:
            'guard auto-read should have recorded mtime; '
    ...
    test('session 1141 exact sequence: guard→read→edit→read→edit', () async {
      ...
      expect(
        r5.output,
        contains('Edit applied'),
        reason:
            'after edit+read+edit+read, a subsequent edit '
    ...
    test('session 1141: guard → read → write using relative paths '
      ...
      expect(
        ...
        contains('File written:'),
        reason:
            'relative path normalization must be consistent '
    ...
    test(
      'tracker state survives simulated app restart via loadSession',
      () async {
        ...
        expect(
          ...
          contains('Edit applied'),
          reason:
              'tracker state loaded from DB must prevent '
