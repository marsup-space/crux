...
void main() {
  ...
  group('pinned row layout — sessions and chats', () {
    test('pinned chat renders inside the Chats section, not above the '
        'Sessions section', () async {
      await testNocterm('pinned chat stays in Chats section', (tester) async {
        ...
        expect(
          ...
          isTrue,
          reason:
              'pinned chat must render below the Chats header '
        ...
        expect(
          ...
          isTrue,
          reason:
              'pinned chat must NOT appear above workspace sessions; '
    ...
    test('pinned session renders inside the Sessions section, above '
        'today\'s sessions', () async {
      await testNocterm('pinned session stays in Sessions section', (
        tester,
      ) async {
    ...
    test('pinned rows are followed by a divider in their own section, '
        'no "Pinned" label is rendered anywhere', () async {
      await testNocterm('divider closes each pinned band, no Pinned label', (
        tester,
      ) async {
        final pinnedSess = makeSession(
          id: 1,
        ...
        expect(
          ...
          isTrue,
          reason:
              'a divider must sit directly below pinned-sess '
        ...
        expect(
          ...
          isTrue,
          reason:
              'a divider must sit directly below pinned-chat '
    ...
    test('pinned chat-only list still renders the Chats header and '
        'its pinned divider', () async {
      ...
      // the pinned band.
      await testNocterm('pinned-only chats still show the Chats section', (
        tester,
      ) async {
        final pinnedChat = makeChat(
          id: 1,
        ...
        await pump(tester, sessions: [todaySess], chats: [pinnedChat]);
        ...
        expect(
          ...
          isTrue,
          reason:
              'divider must sit directly below the pinned chat '
    ...
    test(
      ...
      () async {
        ...
        await testNocterm('no pins = no extra dividers', (tester) async {
          ...
          await pump(tester, sessions: [todaySess], chats: [todayChat]);
          ...
          expect(
            ...
            isTrue,
            reason:
                'a divider must sit directly above today-sess (the '
