...
void main() {
  ...
  group('state diagram parser', () {
    test('start/end markers become circle nodes', () {
      ...
      expect(
        graph.nodes.values.where((n) => n.shape == NodeShape.circle).length,
        2,
      );
...
''');
      ...
      final overlap =
          frontend.y < backend.y + backend.height &&
          backend.y < frontend.y + frontend.height;
      expect(
        overlap,
        isFalse,
        reason:
            'frontend y=${frontend.y} h=${frontend.height}, '
  ...
  group('renderer', () {
    ...
    test('diamond renders as a connected chamfered decision box', () {
      final result = renderDiagram(
        'flowchart TB\nA --> B{Choice} --> C',
        const DiagramRenderOptions(),
        language: 'mermaid',
      ...
      expect(result.text, isNot(contains('┬')));
      // The edge stays outside the frame, so it never shares a colored
      // junction cell with the blue decision border.
    ...
    test('edge labels stay intact beside the path', () {
      ...
      expect(
        result.text.contains('st│rt') || result.text.contains('sto│p'),
        isFalse,
      );
    });
  ...
  group('dispatch', () {
    ...
    test('auto-detect picks mermaid vs d2 by arrow syntax', () {
      ...
      final d2Result = renderDiagram('a -> b', const DiagramRenderOptions());
