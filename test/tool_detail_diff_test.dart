...
void main() {
  group('computeLineDiff', () {
    test('identical inputs collapse to a gap marker', () {
      ...
      expect(lines.firstWhere((l) => l.kind == DiffLineKind.gap).elidedCount, 3);
      expect(
    ...
    test('pure insertion renders added lines with context', () {
      ...
      expect(
        lines.map((l) => l.kind),
        [DiffLineKind.context, DiffLineKind.added, DiffLineKind.context],
      );
    ...
    test('pure deletion renders removed lines with context', () {
      ...
      expect(
        lines.map((l) => l.kind),
        [DiffLineKind.context, DiffLineKind.removed, DiffLineKind.context],
      );
      expect(lines[1].text, 'x');
    });
    ...
    test('long unchanged run between hunks collapses to one gap', () {
      final oldText = ['change1', ...List.filled(20, 'same'), 'change2']
          .join('\n');
      final newText = ['CHANGED1', ...List.filled(20, 'same'), 'CHANGED2']
          .join('\n');
