// Regression test for parallel `edit` calls to the same file.
//
//   EditTool does a non-atomic read → mutate → write on the file's
//   bytes. Without per-file serialization, two concurrent edits to
//   the same file race: both read the original content, both compute
//   their replacement on that original, both write — only the last
//   write survives. The earlier edits are silently lost, and partial
//   writes can corrupt the file. (Verified empirically before the
//   lock was added — see git history for `file_lock.dart`.)
//
//   The fix lives in `lib/src/tools/file_lock.dart`: EditTool wraps
//   its guard check + read-modify-write in `fileLock(resolved).run`,
...
//      edits concurrently. The test asserts that every edit applies
...
void main() {
  group('parallel edit calls to the same file', () {
    ...
    setUp(() async {
      tempDir = await Directory.systemTemp.createTemp('crux_editpar_');
    ...
    /// then batches edits. Without this, the first edit would
    ...
    /// safety of the edit application itself).
    ...
    Future<List<ToolResult>> runEdits(List<Map<String, dynamic>> inputs) {
      ...
      // Fire all edits concurrently — the worst case for a
      // read-modify-write cycle that lacks serialization. If the
      // implementation is correct, every edit's read still sees
      // the previous edit's write (because Dart's event loop on
      // a single isolate naturally interleaves file I/O such that
      // each read completes before the next write, even without
      // explicit locking). If the implementation is wrong, this
      // is where we'll see the race.
      return Future.wait(
