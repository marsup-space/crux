...
void main() {
  group('parseShellMonitorVerdictForTesting — verdict kind', () {
    test('parses bare verdict tokens', () {
      expect(
        parseShellMonitorVerdictForTesting('PROGRESS').kind,
        ShellMonitorVerdictKind.progress,
      );
      expect(
        parseShellMonitorVerdictForTesting('STUCK').kind,
        ShellMonitorVerdictKind.stuck,
      );
      expect(
        parseShellMonitorVerdictForTesting('UNCERTAIN').kind,
    ...
    test('unparsable input fails open to uncertain, never stuck', () {
      expect(
        parseShellMonitorVerdictForTesting('').kind,
        ShellMonitorVerdictKind.uncertain,
      );
      expect(
        parseShellMonitorVerdictForTesting('   ').kind,
      ...
      expect(
        parseShellMonitorVerdictForTesting('I think it is fine').kind,
        ShellMonitorVerdictKind.uncertain,
      );
      // A longer word that merely starts with a verdict token is not
      // a verdict.
      expect(
        parseShellMonitorVerdictForTesting('STUCKER').kind,
        ShellMonitorVerdictKind.uncertain,
      );
    });
  });
  ...
  group('parseShellMonitorVerdictForTesting — interval', () {
    test('defaults to 30s when no interval is given', () {
      expect(
        parseShellMonitorVerdictForTesting('PROGRESS').intervalSeconds,
    ...
    test('parses an interval from the first line', () {
      expect(
        parseShellMonitorVerdictForTesting('PROGRESS 60').intervalSeconds,
        60,
      );
      expect(
        parseShellMonitorVerdictForTesting('STUCK 20').intervalSeconds,
        20,
      );
    });
    ...
    test('clamps the interval to the agreed bounds', () {
      expect(
        parseShellMonitorVerdictForTesting('PROGRESS 1').intervalSeconds,
  ...
  group('parseShellMonitorVerdictForTesting — reason', () {
    ...
    test('parses a same-line reason after the interval', () {
      final v = parseShellMonitorVerdictForTesting(
        'STUCK 5 — waiting on a '
        'Password: prompt it will never receive',
      );
      expect(v.kind, ShellMonitorVerdictKind.stuck);
      expect(v.reason, 'waiting on a Password: prompt it will never receive');
    });
