...
void main() {
  ...
  ToolContext ctxOf(int sessionId, {String? workingDirectory}) => ToolContext(
    sessionId: sessionId,
    messageId: 0,
    abort: AbortSignal(),
  ...
  test('schema exposes the expected actions', () {
    ...
    expect(
      (actions['enum'] as List),
      containsAll(['list', 'show', 'messages', 'search']),
    );
  ...
  test('action=list hides the current session by default', () async {
    ...
    expect(
      result.output,
      isNot(contains('Session B — write the session tool')),
    );
  ...
  test(
    'action=list with includeCurrent=true shows the current session too',
    () async {
      final result = await tool.execute({
        'action': 'list',
        'includeCurrent': true,
  ...
  test(
    'action=list with no other sessions explains the empty result',
    () async {
      ...
      final result = await tool.execute({'action': 'list'}, ctxOf(sessionB));
      expect(result.output, contains('No other sessions found'));
      expect(
        result.output,
        contains('current session ses://$sessionB is hidden'),
      );
  ...
  test(
    'action=show returns metadata + recent messages for another session',
    () async {
      final result = await tool.execute({
        'action': 'show',
        'sessionId': sessionA,
  ...
  test('action=show requires sessionId', () async {
    final result = await tool.execute({'action': 'show'}, ctxOf(sessionB));
  ...
  test('action=messages requires sessionId', () async {
    final result = await tool.execute({'action': 'messages'}, ctxOf(sessionB));
    expect(result.output, contains('Missing required parameter'));
    expect(result.output, contains('sessionId'));
  });
  ...
  test(
    'action=messages default returns the latest N messages, not the oldest',
    () async {
  ...
  test('action=messages beforeId + tight limit paginates correctly', () async {
  ...
  test('action=messages pagination walks a long session correctly', () async {
    ...
    while (true) {
      ...
      if (returned < 3) break;
      final footerMatch = RegExp(r'beforeId=(\d+)').firstMatch(result.output);
  ...
  test(
    'action=show returns the most recent messages, not the oldest',
    () async {
  ...
  test('errors on unknown action', () async {
    final result = await tool.execute({'action': 'teleport'}, ctxOf(sessionB));
