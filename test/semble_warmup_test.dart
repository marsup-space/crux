...
void main() {
  group('SembleWarmup', () {
    ...
    test('start() returns immediately — does NOT block on the warmup', () async {
      ...
      expect(startElapsed.inMilliseconds, lessThan(500),
    ...
    test('start() is idempotent — second call returns the same future',
        () async {
    ...
    test(
      'awaitReady() always completes, even when the underlying '
      'semble call fails (no exception leaks to the agent loop)',
      () async {
    ...
    test(
      'awaitReady() returns the same future as start() — second caller '
      'joins the first warmup instead of triggering a duplicate',
      () async {
    ...
    test('debugReset clears state so the next start() kicks a fresh warmup',
        () async {
    ...
    test(
      ...
      () async {
        ...
        expect(refreshElapsed.inMilliseconds, lessThan(500),
    ...
    test('refresh() is idempotent — concurrent calls collapse to one', () async {
