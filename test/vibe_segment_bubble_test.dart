...
void main() {
  ...
  group('VibeSegmentBubble think box effort label', () {
    ...
    test('falls back to raw internal value when no preset matches', () async {
    ...
    test('falls back to raw value when reasoningPresets is empty', () async {
  ...
  group('VibeSegmentBubble user bubble indentation', () {
    test('user prefix is uppercase `You:` matching verbose mode', () async {
      ...
      await testNocterm('user prefix is uppercase You:', (tester) async {
        ...
        expect(
          tester.terminalState.findText('You:'),
          isNotEmpty,
          reason:
              'vibe mode user prefix must be uppercase '
    ...
    test('user text starts at the same column as crux text', () async {
      ...
      await testNocterm('user + crux column alignment', (tester) async {
        ...
        // verbose mode.
        expect(youLabel!.x, 2, reason: '`You:` prefix must start at column 2');
        expect(
          cruxLabel!.x,
          2,
        ...
        expect(
          ...
          7,
          reason:
              'user text must start at column 7 '
        ...
        expect(
          ...
          8,
          reason:
              'crux text must start at column 8 '
              '(1 padding + 7-char `Crux:` prefix)',
        );
      });
    });

    test('long user message wraps inside Expanded (no flush-left)', () async {
      ...
      await testNocterm('long user wraps inside Expanded', (tester) async {
        ...
        expect(
          ...
          isNotNull,
          reason:
              'expected `the` to be present on the '
        ...
        expect(
          ...
          7,
          reason:
              'wrapped continuation lines must align to '
        ...
        expect(
          the.y,
          isNot(equals(thisWord!.y)),
          reason:
              '`the` must appear on a wrapped '
