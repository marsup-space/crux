...
Future<void> executeView(List<String> parts, CommandContext ctx) async {
  ...
  switch (mode) {
    case 'verbose':
      ...
      ctx.refresh();
      ctx.showToast('Display mode: verbose', mode: ToastMode.status);
    case 'vibe':
      rt.chatDisplayMode = ChatDisplayMode.vibe;
      ctx.persistChatDisplayMode(rt);
      ctx.refresh();
      ctx.showToast('Display mode: vibe', mode: ToastMode.status);
    default:
      ctx.showToast(
        'Unknown mode: "$mode". Usage: /view <verbose|vibe>',
