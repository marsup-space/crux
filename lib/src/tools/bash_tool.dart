...
class BashTool extends ShellBase {
  ...
  String get description =>
      ...
      '  file inspection). '
      ''
      '❌ 🚨 CRITICAL — Tier 3 (general shell, no specific optimization). '
      'NEVER use for anything Tier 1 or Tier 2 covers. '
      'Use the dedicated tool instead: '
      '• `cat`, `head`, `tail`, `less`, `more`            → `read` '
      '• `ls`, `find`, `tree`, `du -sh` on a directory     → `glob` '
      '• `grep`, `rg`, `ack`, `ag` on file contents       → `grep` '
      '• "How does X work / find code that does X"        → `semantic_search` '
      '• "Given a file:line, find code like it"           → `find_similar_code` '
      '• `sed -n "100,120p"`, `awk`, `cut`, `sort`, `uniq` → `read` with '
      '  line range + `grep` '
      '• `wc -l`, `md5sum`, `file`, `stat` on one file    → `read` '
      '• `diff file1 file2`                              → `read` on both '
      ''
      '⚠️ IMPORTANT — WHY: dedicated tools are faster (no shell fork), '
      'return structured output, support parallel calls, and avoid '
      'shell-quoting bugs. Tier 1 tools are highly optimized for one '
      'job — the system prompt lists which tool fits which tier. '
      ''
      'EXAMPLES: '
      ...
      '❌ `rg "auth" lib/ | head`                       → `semantic_search` '
